...
export default {
	...
	methods: {
		...
		dateChange(date, dateString) {
			...
			this.radioVal == 1
				? (this.dateval = `${this.dateValue[0]}-${this.dateValue[1]}`)
						...
						return setGenWeekReport(data).then((res) => {
							if (res.type === "application/json") {
								...
								blobTest.onload = () => {
									this.$message.info(
										`${
											JSON.parse(blobTest.result).data == null
												? "系统错误"
												: JSON.parse(blobTest.result).data
										}`,
									);
								};
								genModal.destroy();
