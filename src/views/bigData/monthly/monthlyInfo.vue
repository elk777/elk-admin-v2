        ...
        <a-tabs style="height:100vh" tab-position="left" @change="callback" v-model="defaultTitle">
            <a-tab-pane v-for="item in tabList" :key="item.id" :tab="item.name">
                    ...
                    <div class="monthlyInfo_manage" v-show="defaultTitle == 1">
                        ...
                        <editable-cell :columns="columnManage"  v-if="manageShow" :ref="item.refName + 'Ref'" :handData="manageData" :editList="editList" :monType="0" @setManagData="setManagData" :isManNull="isManNull" :radioVal="radioVal" :defaultTitle="defaultTitle" :dateval="dateval"></editable-cell>
                    ...
                    <div class="monthlyInfo_examine" v-show="defaultTitle == 2">
                        ...
                        <editable-cell :loading="examineShow" v-if="examineShow" :ref="item.refName + 'safetyRef'" :columns="columns" :handData="examineData" @examineCheck="examineCheck" @setManagData="setManagData" @childrenDel="childrenDel" :editList="examinEditList" :monType="1"></editable-cell>
                    </div>
                    <!-- 领导巡查情况 -->
                    <div class="monthlyInfo_examine" v-show="defaultTitle == 3">
                        <editable-cell v-if="patrolShow" :ref="item.refName + 'leadRef'" :columns="patrolColumns" :handData="patrolData" :editList="patrolEditList" :monType="1" ></editable-cell>
                    ...
                    <div class="monthlyInfo_examine" v-show="defaultTitle == 5">
                        ...
                        <editable-cell v-if="jobmonitorShow" :ref="item.refName + 'highRef'" :columns="jobmonitorColumns" :handData="jobmonitorData" :editList="jobmonitorEditList" :dateval="dateval" :monType="1" @childrenDel="childrenDel" @setManagData="setManagData" ></editable-cell>
                        <div style="height:100vh" v-else>
                            <a-spin class="spinLoading" ></a-spin>
                        </div>
                    </div>
                    ...
                    <div class="monthlyInfo_evaluate monthlyInfo_evaluate_scroll " v-show="defaultTitle == 4">   
                        <div>
                            <a-button style="margin-right:15px" :loading="evaluateLoading"  @click="saveMonthly('evaluate')">保存</a-button>
                            <span>{{ imgInfo }}</span>
                    ...
                    <div class="monthlyInfo_evaluate"  v-show="defaultTitle == 7">
                        ...
                        <editable-cell style="height:342px" v-if="practiceShow" :ref="item.refName + 'practiceRef'" :columns="practiceColumns" :handData="practiceData" :editList="practiceEditList" @upLoad="upLoad" @cententEcho="handelEcho" :monType="2" :uploadShow="1"></editable-cell>
                        <div  v-if="textUploadShow" class="monthlyInfo_evaluate_edit">
                            ...
                            <div v-if="fileList.length < 1 " class="mee_upload">
                                <a-upload accept=".jpg,.jpeg,.png,.gif,.bmp,.JPG,.PBG,.GIF,.BMP"  name="files" :multiple="false" :file-list="fileList" :before-upload="beforeUpload"   :remove="handelRemove"  @preview="handelPreview" >
                                    <div>
                            ...
                            <div v-else class="mee_upload">
                                <div  class="to-img">
                                    <tokenImg :authSrc="previewImage">
                                    </tokenImg>
                                    <div class="tokenimg-class">
                                         <a-icon class="close-icon"   @click=" e => fileList = []" :style="{ fontSize: '25px'}" type="close" />
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
import { getEvaluationList, setEvaluationAdd, EvaluationUpdate, getCheckList, setCheckAdd, checkUpdate, checkDel,setUnitAdd, setTypeAdd, setOtherAdd, setOtherUpdate, getOtherList, setSummaryAdd, getSummaryList, setSummaryUpdate, delSummary, patrolAdd, patrolList, patrolDelete,workList, workAdd,workEdit,workDel }  from "@/api/monthly"
...
export default {
    ...
    methods: {
        ...
        getManagList(){
            const  manageData = [
                { id:null, key:1,engineField:"核电工程安全",finalSocre:100,hierarchy:"一级",norm:"重伤及以上事故",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 0},
                { id:null, key:2,engineField:"核电工程安全",finalSocre:100,hierarchy:"二级",norm:"轻松事件",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 1},
                { id:null, key:3,engineField:"核电工程安全",finalSocre:100,hierarchy:"二级",norm:"未遂事件",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 2},
                { id:null, key:4,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"红线违章",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 3},
                { id:null, key:5,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"高风险作业管控",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 4},
                { id:null, key:6,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"特种设备检验率",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 5},
                { id:null, key:7,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"特种人员持证率",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 6},
                { id:null, key:8,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"人员三级安全教育",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 7},
                { id:null, key:9,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"现场医疗处置事件",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 8},
                { id:null, key:10,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"隐患排查按期整改率",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 9},
                { id:null, key:11,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"关键隐患排查与治理",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 10},
                { id:null, key:12,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"一般以上施工方案及安全技术交底",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 11},
                { id:null, key:13,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"安全生产费用投入",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 12},
                { id:null, key:14,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"典型隐患数量",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 13},
                { id:null, key:15,engineField:"核电工程安全",finalSocre:100,hierarchy:"三级",norm:"停工令(含口头停工)",questionDate:"",reasonDeduction:"",scoreDeduction:"",responsibleUnit:"",implementSituation:"" ,normSort: 14},
            ]
            ...
            getEvaluationList(cahData).then( res => {
        ...
        setManagData(target) {
            ...
            if( type[0] === "reasonDeduction" ) {
                EvaluationUpdate(target).then( res => {
                    if( res.message === 'success') {
                        this.getManagList();
                        this.$refs.manageRef[0].inputChange = false;
                    }
                }).catch( err => { Error(err); this.$refs.manageRef[0].inputChange = false;})
                return
            }
            // 高风险作业监督情况
            if( type[0] == "workInfo" ) {
                workEdit(target).then( res => {
                    if( res.message === 'success') {
                        this.getWorkList();
                        this.$refs.jobmonitorhighRef[0].inputChange = false;
                    }
                }).catch( err => { Error(err); this.$refs.jobmonitorhighRef[0].inputChange = false;})
                return
            }
            // 走的是安全检查情况
            checkUpdate(target).then( res => {
                if( res.message === 'success') {
                    this.getExamineList();
                    this.$refs.examinesafetyRef[0].inputChange = false;
                }
            }).catch( err => {Error(err);this.$refs.examinesafetyRef[0].inputChange = false;})
        },
        // 安全检查情况选择框修改
        ...
        getScoreList() {
            ...
            getOtherList(data).then( res => {
                if( res.data.length !== 0 ){
                    ...
                    res.data.infoImageList.length > 0 ? this.imgInfo = '已上传过图片' : this.imgInfo = '未上传图片'
        ...
        upLoad(uploadData) {
            ...
            if( uploadData.id != null){
                ...
                if( this.fileList.length === 0 ) {
                    ...
                    formdata.append('imgUrl',this.fileList[0].type ? null : this.fileList[0].url.match(/8999(\S*)/)[1])
                }
                formdata.append('id',uploadData.id)
                setSummaryUpdate(formdata).then( res => {
                    if( res.message === 'success') {
                        this.$refs.praxispracticeRef[0].uploading = false;
                        this.$message.success(`修改成功`);
                ...
                setSummaryAdd(formdata).then( res => {
                    if( res.message === 'success') {               
                        this.$refs.praxispracticeRef[0].uploading = false;
...
/* ---------------------------------------------------------------- */
        ...
        unifyUpdateIn(data,id) {
            ...
            setOtherUpdate(formdata).then( res => {
                if( res.message === 'success') {
                    this.evaluateLoading = false
                    this.evaluateFileList = [];
                    this.$message.success('保存成功');
    ...
    /*     unifyUpdateIn(id,addData,editData) {
        ...
        saveMonthly(type) {
            // practiceSummary hazardSource   evaluateFileList
             const { evaluateData:{ id,advantage,inferiority }, evaluateFileList } = this;
            switch (type) {
                //  1_ 安全管理状态
                case 'manage':
                    ...
                    if( this.$refs.manageRef[0].changData.length !== 0  ) {
                        ...
                        setEvaluationAdd(data).then( res => {
                            this.$message.success('保存成功');
                            this.getManagList();
                            this.$refs.manageRef[0].changData = [];
                        }).catch( err => { Error(err); this.manageShow = true;})
                        return
                    }
                    this.$message.success('保存成功');
                    break;
                ...
                case 'examine':
                    ...
                    if( this.$refs.examinesafetyRef[0].changData.length !== 0  ) {
                        ...
                        setCheckAdd(exaData).then( res => {
                            if( res.message === 'success') {
                                this.$message.success('保存成功');
                                this.getExamineList();
                                this.$refs.examinesafetyRef[0].changData = [];
                            }
                        }).catch( err => { Error(err); this.examineShow = true;})
                        return
                    }
                    this.$message.success('保存成功');
                    break;
                ...
                case 'jobmonitor':
                    if( this.$refs.jobmonitorhighRef[0].changData.length !== 0  ) {
                        ...
                        workAdd(exaData).then( res => {
                            if( res.message === 'success') {
                                this.$message.success('保存成功');
                                this.getWorkList();
                                this.$refs.jobmonitorhighRef[0].changData = [];
                            }
                        }).catch( err => { Error(err); this.jobmonitorShow = true;})
                        return
                    }
                    this.$message.success('保存成功');
                    break;
