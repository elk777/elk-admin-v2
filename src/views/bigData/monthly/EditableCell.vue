...
<!-- 公共table组件 -->
    <div class="editable-cell">
        ...
        <a-table  :rowKey="(record,index) => editList[0] ==  'implementation' ? record.id : index" :columns="managecolumns.length == 0 ? columnManage : managecolumns " :data-source="data" bordered size="small" :pagination="false" :scroll="scrollval">
            <template v-for="col in editList" :slot="col" slot-scope="text ,record ,index" >
                <div >
                    <div v-if="record.editable && !uploadShow" style="position:relative">
                        <a-input v-if="col !== 'checkDate' && col !=='imgUrl' && col !== 'beginDate' " :value="text" @change=" e => handelChange(e.target.value,record.id,col,0,index)" />
                        <a-date-picker v-else  @change=" (date,dateString) => handelChange(dateString,index,col,1)" placeholder="请选择检查日期" />
                    </div>
                    <template v-else>
                        {{text}}
                    </template>
             ...
             <template slot="operation" slot-scope="text ,record ,index" >
                <div>
                    <span v-if="record.editable">
                        <a style="margin-right:8px" :class=" uploading ? 'cliUpload' : ''" @click=" () => save(record.id,'',index)">
                            <a-icon v-if="uploading"  type="loading"/>确定
                        </a>
                        <a-popconfirm v-if="montype !== 0" title="是否删除当前行的数据？" :class=" uploading ? 'cliUpload' : ''"   @confirm=" () => cancel(record.id,index)">
                            <a>删除</a>
                        </a-popconfirm>
                    </span>
                    <span v-else>
                        <a :disabled="editingKey !== ''" @click="() => edit(record.id,'',index,record)">编辑</a>
                        <a v-if="defaultTitle && radioVal === 2"  style="margin-left:8px" :disabled="editingKey !== ''" @click="() => add(record.id,'',index)">新增</a>
                    </span>
                </div>
            </template>
        </a-table>
...
export default {
    props:['columns','handData','editList',"monType","uploadShow","defaultTitle",'radioVal','isManNull','dateval'],
    data(){
        return {
            data: this.handData,
            ...
            uploading: false, 
            columnManage:[
                { title:"领域",dataIndex:"engineField",width:120,align:"center",customRender: (value, row, index) => {
                ...
                { title:"最终得分",dataIndex:"finalSocre", width:85,align:"center",customRender: (value, row, index) => {
                    const obj = { children:value,attrs:[]};
                    if( index === 0){
                        obj.attrs.rowSpan = this.data.length;  //这个是个动态的数字
                    }
                ...
                { title:"层级",dataIndex:"hierarchy",width:85,align:"center",customRender: (value, row, index) => {
                    const obj = { children:value,attrs:[]};
                    if(  index === 0 ) {  // 表示一级的第一个 
                        obj.attrs.rowSpan = this.hierarchyObj.oneNum;  //这个是个动态的数字
                    }
                    if( index === this.hierarchyObj.oneNum ) {  // 表示二级的第一个 
                        obj.attrs.rowSpan = this.hierarchyObj.twoNum; //这个是个动态的数字
                    }
                    if( index === this.hierarchyObj.oneNum + this.hierarchyObj.twoNum ) {  // 表示三级的第一个 
                        obj.attrs.rowSpan = this.hierarchyObj.threeNum; //这个是个动态的数字
                    }
                    if( index !== 0 && index !== this.hierarchyObj.oneNum && index !== this.hierarchyObj.oneNum + this.hierarchyObj.twoNum ) {
                        obj.attrs.rowSpan = 0;
                    }
                    return obj
                }},
                { title:"指标",dataIndex:"norm",width:250, align:"center",customRender: (value, row, index) => { 
                ...
                { title:"扣分事由",dataIndex:"reasonDeduction",align:"center",scopedSlots:{ customRender: "reasonDeduction" } },
                { title:"扣分数值",dataIndex:"scoreDeduction",width:85,align:"center",scopedSlots:{ customRender: "scoreDeduction" } },
                { title:"责任单位",dataIndex:"responsibleUnit",width:125,align:"center",scopedSlots:{ customRender: "responsibleUnit" } },
                { title:"落实情况",dataIndex:"implementSituation",width:125,align:"center",scopedSlots:{ customRender: "implementSituation" } },
                { title:"操作",dataIndex:"operation",width:150,align:"center",scopedSlots:{ customRender: "operation" } },
            ],
            hierarchyObj: {
                oneNum: null,
                twoNum: null,
    ...
    methods:{
        ...
        handelChange(value, key, column,type,index) {
            ...
            // 走的是时间选择器
            let target = {};
            key == null ? target = data.find( (item,i) => i === index )  : target = data.find( item => key === item.id )
            if( target ){
                column ===  'pointsNumber' ||  column === 'num' ? target[column] = value.replace(/[^0-9.]/g,'') : target[column] = value;
                this.data = data;
            }
        },
        // 点击修改按钮
        ...
        save(key,editState,index) {
            ...
            if( target && cacheTarget ) {
                ...
                //  表示走安全管理状态 或者 安全检查情况               
                if( this.managecolumns.length === 0 || this.editList[2] === 'questionDescription' ||  this.editList[2] === 'workDept'){
        ...
        cancel(key,index) {
            ...
            let cahtarget = {};
            key == null ? target = data.find( (item,i) => i === index ) : target = data.find( item => key === item.id );
            key == null ? cahtarget = cacheData.find( (item,i) => i === index ) : cahtarget = cacheData.find( item => key === item.id );
            this.editingKey = '';
            // console.log("点击了删除:",key,index,target,cahtarget)
            if( target ) {
                Object.assign(target, cahtarget);
                delete target.editable;
                this.data.splice(data.findIndex( (item,i) => i === index),1);
                // 如果是安全管理状态需要调用合并方法
                if( this.managecolumns.length == 0 ) {
                    this.managDataBmp( this.data );
                    this.managDataNorm("norm");
