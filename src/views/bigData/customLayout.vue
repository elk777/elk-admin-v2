            ...
            <a-row v-for="rowItem in rowArr.row" >
              <a-col class="col_class" :class="colItem.className" v-for="colItem in rowItem.col" :span="colItem.val">
                ...
                <div v-else class="iframe_class">
                  ...
                  <iframe style="z-index: -1;" v-if="iframeShow" :src="colItem.srcURL" id="layout11" sandbox="allow-forms allow-same-origin allow-scripts allow-top-navigation
      ...
      <a-drawer title="自定义展板设置" placement="right" :closable="false" :visible="drawer" :get-container="false"
        ...
        <div class="bodydemo_drawer_top">
          ...
          <div class="bodydemo_drawer_top_layinfo">
            <p v-if="layoutList.length !== 0" >默认方案: <span >{{layoutList[0].scheme_type}}-{{layoutList[0].scheme_name}}</span></p>
            <p >当前方案: <span >{{curveProgram}}</span></p>
            <!-- v-if="curveProgram !== ''"  -->
          </div>
        </div>
    ...
    <a-modal width="485px" :getContainer = "() => $refs.bodydemo"  :visible="layoutModal" :title="layoutTitle" @ok="handleLayout" @cancel="handelCancel"  >
      ...
      <div class="bodydemo_modal_top"  v-if="layoutCut">
        <div>
          ...
          <a-input :disabled="modalRadio !== ''"   v-model="newName" placeholder="请输入方案名称" style="width:373.5px"></a-input>
        </div>
        <div class="bmt_radio"  v-if= " layouAllList.length !== 0 " >
          <a-radio-group  v-model="modalRadio">
            <a-radio class="bmt_radio_sty" :disabled="newName !== ''"   v-for="item in layouAllList" :value="item.scheme_id">
      ...
      <div class="bodydemo_modal_btm" v-else style="position:relative">
        ...
        <a-select placeholder = "请选择方案" v-model="pullset" :allowClear="true" @change="pullChange" style="width:445px" >
  ...
  /* ---------------------------------------------------------------------- */
    ...
    handleLayout() {
      ...
      if( this.layoutCut ){
        ...
        let _name = this.layouAllList.find( item =>{ return item.scheme_id == id }).scheme_name
        let _isdefault = this.layouAllList.find( item =>{ return item.scheme_id == id }).isdefault
      ...
      arr.push(this.layouAllList.find( item => { return  pull == item.scheme_id }))
      if( pull == "" || pull == null  ) {
